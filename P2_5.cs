...
namespace Frame.Chaos
{
    ...
    /// </summary>
    internal class P2_5 : IPluginClient, IGuiExtension
    {
        /// <summary>
        ...
        private Microsoft.Xna.Framework.Matrix getTransMatrixForPoint(Vector3 vecPoint)
        {
            // per default 0 nehmen
            int iIndex = 0;
        ...
        private Vector3 getPointAsVector( int iY,int iX)
        {

            float fX, fY;

            fX = (iX * (_coord.MaxX - _coord.MinX)) / (float)ImageWidth + _coord.MinX;
            fY = (iY * (_coord.MaxY - _coord.MinY)) / (float)ImageHeight + _coord.MinY;

            return new Vector3(fX, fY, 1);
